...
# 已知 C 站类型白名单（其他类型如 AnimateDiff/ONNX/Clip 等目录一律不动）
KNOWN_TYPES = {
    "LORA", "LoRA", "Checkpoint", "VAE", "TextualInversion", "Embedding",
    "ControlNet", "Controlnet", "LoCon", "DoRA", "Hypernetwork",
    "AestheticGradient", "Poses", "Wildcards", "Workflows", "MotionModule",
}

# 环境 → 类型目录映射（WebUI/Forge 与 ComfyUI 目录名不同）
ENV_DIRS = {
    "webui": {
        "LORA": "Lora", "LoRA": "Lora", "LoCon": "Lora", "DoRA": "Lora",
        "Checkpoint": "Stable-diffusion",
        "VAE": "VAE",
        "TextualInversion": "embeddings", "Embedding": "embeddings",
        "ControlNet": "ControlNet", "Controlnet": "ControlNet",
        "Hypernetwork": "hypernetworks",
    },
    "comfyui": {
        "LORA": "loras", "LoRA": "loras", "LoCon": "loras", "DoRA": "loras",
...
# C 站 tags → 分类文件夹（第二级，在 <类型>/<baseModel>/ 之下）；无匹配 tags 的模型保持原位
TAGS_DIRS = [
    (["pose", "posing", "poses", "姿势"], "Pose"),
    (["style", "stylized", "art style", "风格"], "Style"),
    (["character", "characters", "角色"], "Character"),
    (["anime", "manga", "动漫"], "Anime"),
    (["clothing", "outfit", "costume", "服装"], "Clothing"),
    (["concept", "概念"], "Concept"),
    (["background", "backgrounds", "背景"], "Backgrounds"),
    (["celebrity", "real person", "真人"], "Celebrity"),
    (["furry", "兽人"], "Furry"),
    (["mecha", "robot", "机甲"], "Mecha"),
    (["scenery", "landscape", "风景"], "Scenery"),
    (["object", "item", "物件"], "Object"),
    (["3d", "render", "3d渲染"], "3D"),
    (["game", "游戏"], "Game"),
    (["illustration", "artwork", "插画"], "Illustration"),
]
...
def scan_models(root, progress_cb=None):
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=lambda e: None):
        ...
        # 跳过图片缓存目录（<模型名>.images/，里面只有图片没有模型）
        dirnames[:] = [d for d in dirnames if not d.lower().endswith(".images")]
...
def _tags_category(tags):
    """tags（C 站标签列表）→ 分类文件夹名；无匹配返回 None"""
    if not tags:
        return None
    hay = " ".join(str(t).lower() for t in tags)
    for kws, folder in TAGS_DIRS:
...
def _log_move(entry):
    """移动日志（时间/源/目标），供「恢复误整理」回退使用"""
    try:
        import config as _cfg
        log_path = os.path.join(_cfg.APP_DIR, "organize_log.json")
        logs = []
...
def _is_inside(child, parent):
    """child 是否在 parent 之下或等于 parent（路径守卫）"""
    try:
        common = os.path.commonpath([os.path.normpath(child), os.path.normpath(parent)])
...
def organize_model(model_path, models_root, dry_run=True, log_cb=None, rules=None,
                   env="", mode="manual"):
    """整理单个模型。
    mode=manual：不自动移动（返回原位，由前端逐个弹窗选择目标文件夹）。
    mode=rules：自定义规则（关键词→文件夹）命中才移动；未命中保持原位。
    mode=civitai：C 站 tags 自动分类——<类型目录>/<baseModel>/<tags分类>，
        仅处理已知类型；无匹配 tags 保持原位；baseModel 用 C 站原名（光辉/noob/anima 等）。
    ...
    model_type = (meta.get("type") or meta.get("model_type") or "").strip()
    base_model = (meta.get("baseModel") or meta.get("base_model") or "").strip()
        ...
        if rules:
            ...
            for rule in rules:
                ...
                if any(kw in hay for kw in kws):
                    ...
                    break
        if dest_dir is None:
            msgs.append("无规则匹配，保持原位")
            return model_path, msgs
    else:
        # ---- C 站 tags 自动分类（mode=civitai）----
        # 未知类型一律不动
        if model_type not in KNOWN_TYPES:
            msgs.append("未知类型「%s」，不动" % (model_type or "无 info"))
            return model_path, msgs
        # 类型目录：按环境映射；无环境映射时退回旧 TYPE_DIRS
        env_map = ENV_DIRS.get(env) or {}
        tdir = env_map.get(model_type) or TYPE_DIRS.get(model_type) or ""
        if not tdir:
            msgs.append("类型「%s」无目录映射，不动" % model_type)
            return model_path, msgs
        # 第一级：baseModel 原名（光辉/noob/anima…）；无 base 信息只到类型层
        parts = [tdir]
        if base_model and base_model.lower() not in ("other", "unknown", ""):
            parts.append(sanitize_filename(base_model))
        # 第二级：tags 分类；无匹配 tags 保持原位（不丢 Other、不建 Other 目录）
        cat = _tags_category(tags)
        if not cat:
            msgs.append("无匹配 tags，保持原位: %s" % os.path.basename(model_path))
            return model_path, msgs
        parts.append(cat)
        dest_dir = os.path.join(models_root, *parts)

    dest = os.path.join(dest_dir, new_name)

    # ---- 在位守卫：目标与源完全相同 → 不动（防原地重复整理）----
    ...
    if os.path.normpath(dest) == os.path.normpath(model_path):
        ...
        return model_path, msgs
    # 目标目录已是源文件所在目录且文件名不变 → 同样无需整理
    if os.path.normpath(dest_dir) == os.path.normpath(src_dir) and \
            new_name.lower() == os.path.basename(model_path).lower():
        msgs.append("已在目标目录下，无需整理: %s" % os.path.basename(model_path))
        return model_path, msgs
        ...
        for ext in _SIDE_EXTS:
            ...
            if os.path.exists(side):
                try:
                    ...
                    moved_sides.append(os.path.basename(side))
                except Exception as e:
                    msgs.append("附属文件移动失败: %s (%s)" % (os.path.basename(side), e))
        msgs.append("已移动: %s" % new_name)
        _log_move({"time": __import__("time").strftime("%Y-%m-%d %H:%M:%S"),
                   "src": model_path, "dest": dest, "mode": mode,
                   "sides": moved_sides})
...
def _remove_empty_dirs(start, stop_root):
    ...
    # 保护：不删除 models_root 的直接子目录（类型根目录永不删除，防止 Forge/ComfyUI 目录消失）
    if os.path.dirname(start) == stop_root:
        return
    while start != stop_root and os.path.dirname(start) != stop_root:
        try:
            os.rmdir(start)
        except OSError:
            return
